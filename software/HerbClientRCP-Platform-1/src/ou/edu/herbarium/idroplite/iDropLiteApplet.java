...
public class iDropLiteApplet {
    ...
    PutTransferRunner currentTransferRunner;
    ...
    protected void getAppletParams() {
		this.host = "data.iplantcollaborative.org";
		this.port = 1247;
		this.user = "yiming";
		this.zone = "iplant";
		this.tempPswd = "Zaq1@wsx";
    ...
    private boolean retrievePermAccount() {//Current File
        String pswd = null;
    ...
    private boolean createPermAccount() {
    	System.out.println(host+"*"+port+"*"+user+"*"+tempPswd+"*"+absPath+"*"+zone+"*"+defaultStorageResource);
...
//        lblZone.setText(this.getIrodsAccount().getZone());//login
...
//    }
    ...
    public void showIdropException(Exception idropException) {
        System.out.println(idropException.getMessage()+ " iDROP Exception "+ JOptionPane.WARNING_MESSAGE);
    }
...
//        System.out.println("This is a test"+org.openide.util.NbBundle.getMessage(iDropLiteApplet.class, "iDropLiteApplet.btnUploadBeginImport.text"));
...
//        btnUploadBeginImport.setMinimumSize(new java.awt.Dimension(92, 34));
//        btnUploadBeginImport.setPreferredSize(new java.awt.Dimension(92, 38));
...
//
//        pnlRestart.setBorder(javax.swing.BorderFactory.createEmptyBorder(1, 60, 1, 60));
//        pnlRestart.setLayout(new java.awt.BorderLayout());
//
...
//        pnlRestart.add(jLabel3, java.awt.BorderLayout.CENTER);
//
...
//    private javax.swing.JPanel pnlRestart;
...
//    }
    ...
    public void run(String uploadDir) throws JargonException{
    	iDropLiteApplet applet = new iDropLiteApplet();
    	applet.getAppletParams();
    	applet.doStartup();
    	IRODSFileSystem system = applet.iDropCore.getIrodsFileSystem();
    	CollectionAndDataObjectListingEntry root = new CollectionAndDataObjectListingEntry();
    	StringBuilder sb = new StringBuilder();
        sb.append("/");
        sb.append(applet.getIrodsAccount().getZone());
        sb.append("/");
        sb.append("home");
        root.setParentPath(sb.toString());
        root.setPathOrName(applet.getIrodsAccount().getHomeDirectory());
        root.setObjectType(CollectionAndDataObjectListingEntry.ObjectType.COLLECTION);
        
        CollectionAndDataObjectListAndSearchAO collectionAO = applet.irodsFileSystem.getIRODSAccessObjectFactory().getCollectionAndDataObjectListAndSearchAO(applet.irodsAccount);
        List<CollectionAndDataObjectListingEntry> childCache = collectionAO.listDataObjectsAndCollectionsUnderPath("/iplant/home/yiming");
        for(CollectionAndDataObjectListingEntry e : childCache){
        	System.out.println("**"+e.getPathOrName());
        	List<CollectionAndDataObjectListingEntry> subCache = collectionAO.listDataObjectsAndCollectionsUnderPath(e.getPathOrName());
	    	for(CollectionAndDataObjectListingEntry ee : subCache){
	    		System.out.println("**	"+ee.getPathOrName());
	    	}
        }
        List<UploadDataObj> uploadData = new ArrayList<UploadDataObj>();
        File testUploadFile = new File("C:/workspace/upload");
        for (File file : testUploadFile.listFiles()) {
        	uploadData.add(new UploadDataObj(file));
        }
        applet.currentTransferRunner = new PutTransferRunner(applet.iDropCore, "/iplant/home/yiming/TestUpload", uploadData);
		final Thread transferThread = new Thread(applet.currentTransferRunner);
		log.info("launching transfer thread");
		transferThread.start();
    }
