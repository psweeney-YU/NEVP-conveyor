...
import ou.edu.herbarium.dialog.CameraErrorDialog;
...
//						return;
					...
					if(info.startsWith("CURRENT_SPECIMEN:")){
						System.out.println("info="+info);
						parent.getDisplay().asyncExec(new Runnable() {
							public void run() {
								for(int i=0;i<table2.getItems().length;i++){
									TableItem item = table2.getItem(i);
									if(item.getData("specimen")!=null){
										table2.remove(i);
										break;
									}
								}
								TableItem newItem = new TableItem(table2,SWT.FULL_SELECTION);
								newItem.setImage(0,ImageFactory.loadImage(parent.getDisplay(), ImageFactory.INFO));
								newItem.setText(1, "Imaging Specimen : "+info.replaceAll("CURRENT_SPECIMEN:", ""));
								newItem.setData("specimen", "specimen");
							}
						});
						continue;
					}
							...
							try{
								for(int i=0;i<list.size();i++){
									if(list.get(i)==number-1000){
										...
										Display.getDefault().asyncExec(rr);
										list.remove(i);
										continue;
									}
								}
