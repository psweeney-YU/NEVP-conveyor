...
//					final TransferData td = sdata.getShareChar();
					...
					display.asyncExec(new Runnable() {
						public void run() {
							...
							Specimen loadSpec = null;
							while(true){
								loadSpec = delegate.getSpecimenById(spec.getSpecimenId());
								if(loadSpec != null) break;
							}
							if(loadSpec==null) System.out.println("Warning!!"+spec.getSpecimenId());
							loadSpec.setTempData(spec.getTempData());
							loadSpec.setWorkspace(spec.getWorkspace());
							loadSpec.setIsFolder(spec.getIsFolder());
							loadSpec.setFolderPath(data8.getOriginalFilePath());
							loadSpec.setNext(spec.isNext());
							Collector collector = new Collector();
							String [] tmpArray = loadSpec.getTempData().split("#");
							String name = tmpArray[0];
							String RecordNum = "";
							if(tmpArray.length>1){
								RecordNum = loadSpec.getTempData().split("#")[1];
							}
...
//							loadSpec.getSpecCollectorMaps().add(scm);
							...
							imageItem.setText( loadSpec.getBarcode());
							imageItem.setData(loadSpec);
							if(loadSpec.getMissingInfo()!=null&&loadSpec.getMissingInfo()==1){
								imageItem.setBackground(new Color(Display.getCurrent(),255,255,200));
							}
							else if(loadSpec.isNext()==true){
								imageItem.setBackground(new Color(Display.getCurrent(),180,255,180));
							}
							imageItem.setData("path",loadSpec.getImagePath());
						}
					});
				}
			}
