	...
	protected void buttonPressed(int buttonId) {
		if(LoginDialog.LOGIN_ID == buttonId){
			...
			try{
				...
				user = delegate.validateLogin(username.getText(), password.getText());
				if(user==null){
					MessageDialog.openError(this.getParentShell(), "Login Error", "Invalid username or password");
					Activator.userId = -1;
					close();
					return;
				}
					...
					Activator.userId = user.getUserId();
					java.lang.System.out.println("Activator.userId"+Activator.userId);
					List<Institute> inss = delegate.getInstitutes();
					ArrayList<SystemIns> ps = new ArrayList<SystemIns>();
					
					for(Institute ins : inss){
						int instituteId = ins.getInstituteId();
						List<System> platforms = delegate.getSystems(instituteId);
						for(System s : platforms){
							ps.add(new SystemIns(s,ins.getInstituteName()));
						}
					}
					
					SystemSelectDialog dlgg = new SystemSelectDialog(this.getParentShell(),ps);
					dlgg.open();
				}
			}catch(Exception e){
				e.printStackTrace();
				MessageDialog.openError(this.getParentShell(), "Connection Error", "Can not connect to server or database, Please check the connection.");
			}
