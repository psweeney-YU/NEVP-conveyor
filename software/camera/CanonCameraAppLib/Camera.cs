    ...
    public sealed class Camera
    {
            ...
            private uint objectEventHandler(uint inEvent, IntPtr inRef, IntPtr inContext)
            {
                switch (inEvent)
                {
                    case EDSDK.ObjectEvent_DirItemCreated:
                        ...
                        Console.WriteLine("Directory Item Created" + inRef);
                        break;
        ...
        #region Public Methods
            ...
            public void takePhotograph()
            {
                ...
                UdpClient udpClient;
                Byte[] sendBytes = Encoding.ASCII.GetBytes("ERROR NO = " + error);
                int sentLength = 0;
                switch (error)
                {
                    case EDSDK.EDS_ERR_OK:
                    ...
                    default:
                        ...
                        udpClient = new UdpClient(11000);
                        udpClient.Connect("localhost", 60005);
                        sendBytes = Encoding.ASCII.GetBytes("ERROR NO = " + error);
                        sentLength = udpClient.Send(sendBytes, sendBytes.Length);
                        if (sentLength > 0) udpClient.Close();
                        break;
            ...
            public void deleteCapturedItem(IntPtr directoryItem)
            {
                uint err = EDSDK.EDS_ERR_OK;
                err = EDSDK.EdsDeleteDirectoryItem(directoryItem);
                if (err != EDSDK.EDS_ERR_OK)
                {
                    throw new CameraException("Unable to delete item!", err);
                }
            }
