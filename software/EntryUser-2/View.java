...
public class View extends ViewPart {
	 ...
	 * it.
	 */
	public static void updateGallery(List<Specimen> specimens, Gallery gallery,
			String type,DataUtilsDelegate delegate){
		gallery.removeAll();
		int tag = 0;
		for(Specimen spec : specimens){
			...
			boolean added = false;
			System.out.println("SPECIMENS="+spec.getBarcode()+"*"+spec.getScientificName()+"*"+spec.getRecordNumber()+"*"+spec.getFolderName());
...
//								itemFamily.setText(1, spec.getFamily());
		...
		updateGallery(specimens, gallery, "",delegate);
		combo.addSelectionListener(new SelectionListener(){
			...
			public void updateGallery(List<Specimen> specimens, Gallery gallery,String type){
				...
				for(Specimen spec : specimens){
					...
					boolean added = false;
					String qrcode = spec.getImageRaw().getImageRawPath();//fake
					for(GalleryItem item : gallery.getItems()){
						String itemQR = item.getText();
						if(itemQR.equals(qrcode)){
							...
							imageItem.setText( spec.getBarcode());
							if(spec.getMissingInfo()!=null&&spec.getMissingInfo()==1){
								imageItem.setForeground(new Color(Display.getCurrent(),255,255,200));
//								imageItem.setBackground(new Color(Display.getCurrent(),255,255,200));
							}
							if(spec.getImageError()!=null&&spec.getImageError()==1){
								imageItem.setForeground(new Color(Display.getCurrent(),255,200,200));
							}
							imageItem.setData(spec);
							imageItem.setData("path",spec.getImagePath());
							GalleryItem [] items = new GalleryItem [] {imageItem};
